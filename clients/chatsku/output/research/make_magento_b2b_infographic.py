"""
Infographic for: magento-b2b-chatbot-integration (2026-07-06)
860x452, white background, ChatSKU palette (accent #00C9B1, navy #1a1a2e, red #e94560).
Before/after after-hours conversion bar chart for the 18,000-SKU Adobe Commerce distributor example.
"""
...
d.text((32, 18), "Magento B2B store: before vs after ChatSKU", font=F(BOLD, 24), fill=WHITE)
d.text((32, 50), "18,000 SKUs. 1,500 after-hours sessions/month. $1,800 AOV.", font=F(REG, 14), fill=ACCENT)
...
# Before bar
bv = 48600; bh = bar_h(bv); bx = left_x
d.rectangle([bx, base_y - bh, bx + bar_w, base_y], fill=GREY)
d.text((bx - 4, base_y + 12), "BEFORE", font=F(BOLD, 14), fill=INK)
d.text((bx - 4, base_y + 33), "1.8% conversion", font=F(REG, 13), fill=GREY)
d.text((bx, base_y - bh - 30), "$48,600/mo", font=F(BOLD, 21), fill=INK)

# After bar
av = 97200; ah = bar_h(av); ax = left_x + bar_w + 80
d.rectangle([ax, base_y - ah, ax + bar_w, base_y], fill=ACCENT)
d.text((ax - 4, base_y + 12), "AFTER", font=F(BOLD, 14), fill=NAVY)
d.text((ax - 4, base_y + 33), "3.6% conversion", font=F(REG, 13), fill=ACCENT)
d.text((ax, base_y - ah - 30), "$97,200/mo", font=F(BOLD, 21), fill=ACCENT)
...
d.text((gx + 24, 188), "$48,600", font=F(BOLD, 32), fill=NAVY)
d.text((gx + 24, 230), "more every month", font=F(REG, 14), fill=GREY)
d.text((gx + 24, 262), "$583,200 / year", font=F(BOLD, 19), fill=RED)
...
out = os.path.join(os.environ.get("OUT_DIR", os.getcwd()), "magento_b2b_infographic_860x452.jpg")
