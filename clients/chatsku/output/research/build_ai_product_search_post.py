...
DRAFT = r"C:\content-intel\clients\chatsku\output\drafts\how-ai-product-search-works-2026-08-18.md"
YOAST_TITLE = "How AI Product Search Works for B2B | ChatSKU"          # 44 chars
YOAST_DESC = ("AI product search turns your catalog into vectors, runs keyword and "
              "meaning-based retrieval together, and answers only from real records. "
              "Here is how it works.")                                  # 158 chars
PUBLISH_DATE = "2026-08-18"
CATEGORY_ID = 29                               # Chatbot -- never 25 (DFW Local)

STATE_PATH = r"C:\content-intel\clients\chatsku\output\research\aips_push_state.json"
...
FEATURED_ALT = ("Bearded manager in a blue blazer at a glass-walled office desk, pen to his mouth, "
                "studying a laptop while running a product search")
BODY1_ALT = ("Close-up of hands typing on a laptop showing a colored inventory grid, with a machined "
             "industrial component resting on the desk in front")
BODY2_ALT = ("Tall warehouse pallet racking stacked with shrink-wrapped labelled cartons, shot from "
             "below, the real stock behind every catalog record")
...
for u in ["https://chatsku.com/demo/"]:
    r = requests.get(u, headers=HG, timeout=30, allow_redirects=False)
    ...
    for kind, title, body in parsed:
        ...
        if kind == "exec":
            ...
            widgets = [w_heading(title, color="#ffffff", align="center", element_id=eid),
                       ...
                       w_button("See the live demo", "https://chatsku.com/demo/")]
...
if not DRY_RUN:
    ...
    if st.get("featured_media"):
        ...
        FEATURED = upload_media(read_qa_image("featured"),
                                "chatsku-how-ai-product-search-works-featured.jpg",
    ...
    if st.get("body1_media"):
        ...
        BODY1 = upload_media(read_qa_image("body1"),
                             "chatsku-how-ai-product-search-works-body1.jpg",
    ...
    if st.get("body2_media"):
        ...
        BODY2 = upload_media(read_qa_image("body2"),
                             "chatsku-how-ai-product-search-works-body2.jpg",
...
open(os.path.join(SCRATCH, "aips_elementor.json"), "w", encoding="utf-8").write(elementor_json)
...
if not DRY_RUN and img_widget_count != 2:
...
if not FEATURED or not FEATURED.get("id"):
