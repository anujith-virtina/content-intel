"""
Build + publish: b2b-chatbot-for-woocommerce
"How to add a B2B chatbot to your WooCommerce store"
Format B (Conversational Q&A) blended with How-To.   Date: 2026-06-29

Parses the approved draft, splits <h3> into heading widgets, styles tables (post 299 gold
standard), renders FAQ as Elementor native accordion (post 299 standard), adds
Article + HowTo + FAQPage + BreadcrumbList JSON-LD.
Images (all 860x452): featured + 3 body photos (buyer, admin, team) + 1 infographic.
Env: DRY_RUN=1, REUSE_MEDIA="feat,buyer,admin,team,info", UPDATE_POST_ID, FORCE_STATUS
"""
...
DRAFT = r"C:\content-intel\clients\chatsku\output\drafts\b2b-chatbot-for-woocommerce-2026-06-29.md"
...
DATE = "2026-06-29"
META_TITLE = "B2B Chatbot for WooCommerce | ChatSKU"
META_DESC = ("Add a B2B chatbot for WooCommerce that reads your catalog, shows tiered pricing, and "
             "builds quotes in chat. See the 7 steps, real costs, and ROI for B2B stores.")
...
EXPECTED = [
    ...
    "What is a B2B chatbot for WooCommerce?",
    "Why do WooCommerce B2B stores need a chatbot more than B2C?",
    "What can a B2B chatbot actually do on WooCommerce?",
    "How do you add a B2B chatbot to your WooCommerce store?",
    "What about WooCommerce B2B plugins? Do you still need a chatbot?",
    "How long does it take to add a B2B chatbot to WooCommerce?",
    "How much does a B2B chatbot for WooCommerce cost?",
    "A real example: a WooCommerce store before and after a B2B chatbot",
    "Is your WooCommerce store ready for a B2B chatbot? A quick check",
...
def build_schema(feat_url):
    ...
    deploy = sections["How do you add a B2B chatbot to your WooCommerce store?"]
    steps = []
    ...
    howto = {"@context": "https://schema.org", "@type": "HowTo",
             "name": "How to add a B2B chatbot to your WooCommerce store", "step": steps}
    blocks, types = [], []
...
FEAT_SRC = os.path.join(SCRATCH, "wc_featured_1.jpg")
BUYER_SRC = os.path.join(SCRATCH, "wc_buyer_0.jpg")
ADMIN_SRC = os.path.join(SCRATCH, "wc_admin_5.jpg")
TEAM_SRC = os.path.join(SCRATCH, "wc_team_9.jpg")
INFO_SRC = os.path.join(SCRATCH, "woocommerce_b2b_infographic_860x452.jpg")
FEAT_ALT = ("WooCommerce store dashboard open on a laptop as a B2B distributor reviews product catalog "
            "and order data at an office desk")
BUYER_ALT = ("B2B purchasing manager searching a WooCommerce store catalog on a laptop after hours to find "
             "a specific product, price, and lead time")
ADMIN_ALT = ("WooCommerce admin dashboard scene on a desktop screen showing product catalog and store data "
             "while setting up a B2B chatbot")
TEAM_ALT = ("B2B sales team reviewing buyer conversations and quote requests handed off from a WooCommerce "
            "AI catalog assistant during the day")
INFO_ALT = ("Before and after bar chart showing WooCommerce after-hours conversion rising from 1.6 to 3.4 "
            "percent, a 158,760 dollar yearly gain")
...
if os.environ.get("DRY_RUN") and not REUSE:
    ...
    buyer = {"id": 9002, "url": "https://chatsku.com/wp-content/uploads/2026/06/b1.jpg", "alt": BUYER_ALT}
    admin = {"id": 9003, "url": "https://chatsku.com/wp-content/uploads/2026/06/b2.jpg", "alt": ADMIN_ALT}
    team = {"id": 9004, "url": "https://chatsku.com/wp-content/uploads/2026/06/b3.jpg", "alt": TEAM_ALT}
    info = {"id": 9005, "url": "https://chatsku.com/wp-content/uploads/2026/06/i.jpg", "alt": INFO_ALT}
    print("DRY_RUN images")
elif REUSE:
    fid, bid, aid, tid, iid = REUSE.split(",")
    feat, buyer, admin, team, info = (fetch_media(fid, FEAT_ALT), fetch_media(bid, BUYER_ALT),
                                      fetch_media(aid, ADMIN_ALT), fetch_media(tid, TEAM_ALT),
                                      fetch_media(iid, INFO_ALT))
    print(f"Reusing media: {fid},{bid},{aid},{tid},{iid}")
else:
    print("\nUploading images...")
    feat = upload(resize_file(FEAT_SRC), "chatsku-b2b-chatbot-for-woocommerce-featured.jpg", FEAT_ALT)
    buyer = upload(resize_file(BUYER_SRC), "chatsku-woocommerce-b2b-buyer-laptop.jpg", BUYER_ALT)
    admin = upload(resize_file(ADMIN_SRC), "chatsku-woocommerce-admin-dashboard.jpg", ADMIN_ALT)
    team = upload(resize_file(TEAM_SRC), "chatsku-woocommerce-b2b-sales-team.jpg", TEAM_ALT)
    info = upload(resize_file(INFO_SRC), "chatsku-woocommerce-before-after-roi.jpg", INFO_ALT)

imgs = [feat, buyer, admin, team, info]
for lbl, m in zip(["feat", "buyer", "admin", "team", "info"], imgs):
    if not m["url"].startswith("https://chatsku.com/wp-content/uploads/"):
        print(f"FATAL {lbl} url: {m['url']}"); sys.exit(1)
print(f"Images: feat={feat['id']} buyer={buyer['id']} admin={admin['id']} team={team['id']} info={info['id']}")

# ---- build sections ----
BG = {
    "Executive summary": "#f9f9fb", "Introduction": "#ffffff",
    "What is a B2B chatbot for WooCommerce?": "#f0f4ff",
    "Why do WooCommerce B2B stores need a chatbot more than B2C?": "#ffffff",
    "What can a B2B chatbot actually do on WooCommerce?": "#f9f9fb",
    "How do you add a B2B chatbot to your WooCommerce store?": "#ffffff",
    "What about WooCommerce B2B plugins? Do you still need a chatbot?": "#f0f4ff",
    "How long does it take to add a B2B chatbot to WooCommerce?": "#ffffff",
    "How much does a B2B chatbot for WooCommerce cost?": "#f9f9fb",
    "A real example: a WooCommerce store before and after a B2B chatbot": "#ffffff",
    "Is your WooCommerce store ready for a B2B chatbot? A quick check": "#f0f4ff",
    "Conclusion": "#1a1a2e", "Frequently asked questions": "#f9f9fb",
}
IMG_AFTER = {
    "Why do WooCommerce B2B stores need a chatbot more than B2C?": buyer,
    "What can a B2B chatbot actually do on WooCommerce?": team,
    "How do you add a B2B chatbot to your WooCommerce store?": admin,
    "A real example: a WooCommerce store before and after a B2B chatbot": info,
}
elementor = []
...
EXISTING_SLUGS = ["rfq-automation-manufacturers", "rfq-automation-for-product-catalogs",
    ...
    "passive-catalog-costing-you-sales"]
...
for l, a in zip(["feat", "buyer", "admin", "team", "info"], alts): print(f"      alt[{l}] {len(a)}: {a}")
...
# 3+ existing-blog interlinks required
BLOG_SLUGS = ["what-is-a-b2b-catalog-chatbot", "b2b-after-hours-buyer-problem", "b2b-conversational-commerce",
              "rfq-form-conversion-rate", "b2b-catalog-conversion-rate", "best-b2b-catalog-chatbots-2026"]
...
# unique anchor text
anchor_texts = re.findall(r'<a [^>]*>(.*?)</a>', ALL_TEXT, flags=re.S)
anchor_texts = [strip_tags(a).lower() for a in anchor_texts]
...
checks["Definition first sentence"] = sections["What is a B2B chatbot for WooCommerce?"].lstrip().startswith(
    "<p>A B2B chatbot for WooCommerce is ")
checks["Primary kw in title"] = "b2b chatbot" in TITLE.lower() and "woocommerce" in TITLE.lower()
checks["Primary kw in meta desc"] = "b2b chatbot for woocommerce" in META_DESC.lower()
...
pub = (f'<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">\n<title>{META_TITLE}</title>'
       ...
       f'<!-- Featured: {feat["id"]} | Buyer: {buyer["id"]} | Admin: {admin["id"]} | Team: {team["id"]} | Infographic: {info["id"]} | Status: draft -->\n'
...
out_path = Path(r"C:\content-intel\clients\chatsku\output\published\b2b-chatbot-for-woocommerce-2026-06-29.html")
out_path.write_text(pub, encoding="utf-8")
print(f"  Saved: {out_path}")
print("\n" + "=" * 60)
print(f"RESULTS_JSON={json.dumps({'post_id': post_id, 'status': v.get('status'), 'feat': feat['id'], 'buyer': buyer['id'], 'admin': admin['id'], 'team': team['id'], 'info': info['id'], 'sections': len(elementor), 'words': wc, 'internal': len(int_links), 'external': len(ext_links), 'schema': schema_types, 'howto_steps': n_steps})}")
