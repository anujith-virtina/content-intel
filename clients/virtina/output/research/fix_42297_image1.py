"""
Fix Image 1 (A-warehouse.jpg) in the PUBLISHED post 42297:
- Change 860x452 -> 670x352
- Add style="aspect-ratio: auto 670 / 352; max-width: 100%;"
Works on the live published content, not our local draft.
"""
import os, re, requests, urllib3
from dotenv import load_dotenv

urllib3.disable_warnings()
load_dotenv()

WP_BASE = "https://virtina.com"
auth = (os.getenv("WP_USERNAME"), os.getenv("WP_APP_PASSWORD"))
POST_ID = 42297

orig_put = requests.put
requests.put = lambda *a, **kw: orig_put(*a, **{**kw, "verify": False})
orig_get = requests.get
requests.get = lambda *a, **kw: orig_get(*a, **{**kw, "verify": False})

# Fetch live published content
resp = requests.get(f"{WP_BASE}/wp-json/wp/v2/posts/{POST_ID}?context=edit", auth=auth)
d = resp.json()
raw = d["content"]["raw"]
status = d["status"]
print(f"Fetched: {len(raw)} chars, status={status}")

# Fix the warehouse image tag
old_img = '<img src="https://virtina.com/wp-content/uploads/2026/05/A-warehouse.jpg" alt="Warehouse employee reviewing and signing a purchase order beside a desktop computer displaying an online checkout/order management screen in a large warehouse setting with shelves and inventory in the background." width="860" height="452">'

new_img = '<img src="https://virtina.com/wp-content/uploads/2026/05/A-warehouse.jpg" alt="Warehouse employee reviewing and signing a purchase order beside a desktop computer displaying an online checkout/order management screen in a large warehouse setting with shelves and inventory in the background." width="670" height="352" style="aspect-ratio: auto 670 / 352; max-width: 100%;">'

if old_img in raw:
    raw = raw.replace(old_img, new_img, 1)
    print("  APPLIED: Image 1 fixed — 860x452 -> 670x352 + style added")
else:
    # Regex fallback
    pat = re.compile(
        r'<img([^>]*)src="([^"]*A-warehouse[^"]*)"([^>]*)width="860"([^>]*)height="452"([^>]*)>',
        re.DOTALL
    )
    def fix_img(m):
        tag = m.group(0)
        tag = re.sub(r'width="\d+"', 'width="670"', tag)
        tag = re.sub(r'height="\d+"', 'height="352"', tag)
        # Add style before closing >
        tag = tag.rstrip('>')
        tag += ' style="aspect-ratio: auto 670 / 352; max-width: 100%;">'
        return tag
    raw, n = pat.subn(fix_img, raw, count=1)
    if n:
        print(f"  APPLIED: Image 1 fixed (regex, {n} match)")
    else:
        print("  MISS: Image not found — check manually")
        exit(1)

# Verify all images
print("\n[IMAGE CHECK after fix]")
for i, m in enumerate(re.finditer(r'<img[^>]+>', raw, re.DOTALL), 1):
    tag = m.group()
    src = re.search(r'src="([^"]+)"', tag)
    w   = re.search(r'width="([^"]+)"', tag)
    h   = re.search(r'height="([^"]+)"', tag)
    sty = re.search(r'style="([^"]+)"', tag)
    print(f"  Img {i}: {(src.group(1)[-45:] if src else '?')} | {w.group(1) if w else '?'}x{h.group(1) if h else '?'} | style={'yes' if sty else 'MISSING'}")

# Push back as publish (post is already live)
print(f"\nPUTting to post {POST_ID} (status: {status})...")
resp = requests.put(
    f"{WP_BASE}/wp-json/wp/v2/posts/{POST_ID}",
    auth=auth,
    json={"content": raw, "status": status}
)
print(f"HTTP {resp.status_code}")
if resp.status_code in (200, 201):
    print(f"Modified: {resp.json().get('modified', '')}")
    print("DONE")
else:
    print("ERROR:", resp.text[:400])
