...
ALT_TEXT = (
    "B2B procurement manager reviewing product catalog and order history "
    "inside a WooCommerce customer portal on a desktop computer in an office"
)

SOURCES = ["rawpixel", "flickr", "nappy", "wordpress"]

QUERIES = [
    "professional laptop ecommerce office",
    "business person computer screen office",
    "professional desk laptop work",
    "business woman laptop office desk",
    "man computer screen office professional",
]
...
def is_relevant(item):
    ...
    bad_words = {"nature", "flower", "landscape", "animal", "food", "wedding",
                 "warehouse", "factory", "worker", "vest", "industrial"}
    if any(w in title for w in bad_words):
        return False
    return True

def try_rawpixel_image_url(url):
    """Convert rawpixel editor_1024 thumbnail URL to image_1300 full-res URL."""
    urls = []
    if "editor_1024" in url:
        urls.append(url.replace("editor_1024", "editor_2048"))
        urls.append(url.replace("editor_1024", "editor_original"))
    # Try image_1300 pattern (different rawpixel URL scheme)
    if "rawpixel.com" in url:
        # Extract image ID from URL and try image_1300 CDN path
        import re as _re
        m = _re.search(r'/image_\d+/([^/?]+)', url)
        if not m:
            m = _re.search(r'/editor_\d+/([^/?]+)', url)
        if m:
            fname = m.group(1)
            # image_1300 pattern seen to work for rawpixel
            urls.append(f"https://images.rawpixel.com/image_1300/{fname}")
    urls.append(url)
    return urls
...
for query in QUERIES:
    for source in SOURCES:
        ...
        for item in items:
            ...
            if not url:
                continue
            urls_to_try = try_rawpixel_image_url(url) if "rawpixel" in url else [url]
            print(f"  {url[:65]}")
            result = None
...
print(f"\nSelected: {chosen_url}")
media_id, src_url = upload_media(img_bytes, "woocommerce-b2b-portal-body1-670x352-v2.jpg")
if not media_id:
    print("Upload failed.")
    exit(1)
...
html_path = os.path.join(os.path.dirname(__file__), "final_html_v5.html")
with open(html_path, "r", encoding="utf-8") as f:
    content = f.read()

# Replace old body1 span (matched by data-id=42207)
old_pat = re.compile(r'<span[^>]*>\s*<img[^>]*data-id="42207"[^>]*>\s*</span>', re.DOTALL)
if old_pat.search(content):
    content = old_pat.sub(new_span, content)
    print("Replaced body1 span (by data-id=42207)")
else:
    old_src = re.compile(r'<span[^>]*>\s*<img[^>]*body1[^"]*"[^>]*>\s*</span>', re.DOTALL)
    if old_src.search(content):
        content = old_src.sub(new_span, content)
        print("Replaced body1 span (by src filename)")
    else:
        print("WARNING: could not find old body1 span")
        exit(1)

v6_path = os.path.join(os.path.dirname(__file__), "final_html_v6.html")
with open(v6_path, "w", encoding="utf-8") as f:
    f.write(content)
print("Saved final_html_v6.html")
...
if resp.status_code == 200:
    ...
    print(f"Old ID 42207 gone: {'data-id=\"42207\"' not in rendered}")
