#!/usr/bin/env python3
"""
Virtina WooCommerce B2B Customer Portal - Complete Publish Pipeline
Handles: image sourcing, upload, HTML build, WordPress post push
"""
import os
import sys
import json
...
if not WP_USERNAME or not WP_APP_PASSWORD:
    print("ERROR: WP_USERNAME or WP_APP_PASSWORD not set in .env")
    sys.exit(1)

auth_b64 = base64.b64encode(f"{WP_USERNAME}:{WP_APP_PASSWORD}".encode()).decode()
WP_AUTH = f"Basic {auth_b64}"
WP_MEDIA = "https://virtina.com/wp-json/wp/v2/media"
WP_POSTS = "https://virtina.com/wp-json/wp/v2/posts"
...
IMAGES = [
    {
        "name": "featured", "w": 1309, "h": 500,
        "queries": ["business professional laptop office", "office worker laptop computer desk", "business desk laptop work professional"],
        "alt": "Business professional reviewing a B2B customer portal dashboard on a laptop in a modern office workspace",
        "fn": "woocommerce-b2b-portal-featured.jpg",
    },
    {
        "name": "body1", "w": 670, "h": 352,
        "queries": ["warehouse worker shelves", "warehouse inventory storage aisle", "warehouse worker boxes storage"],
        "alt": "Warehouse worker in high-visibility vest checking inventory shelves representing B2B order and stock management",
        "fn": "woocommerce-b2b-portal-body1.jpg",
    },
    {
        "name": "body2", "w": 670, "h": 352,
        "queries": ["business team meeting laptop", "team meeting table documents", "office meeting discussion laptop"],
        "alt": "Small business team reviewing WooCommerce portal implementation plans at a meeting table with laptop and documents",
        "fn": "woocommerce-b2b-portal-body2.jpg",
    },
    {
        "name": "body3", "w": 670, "h": 352,
        "queries": ["business phone office laptop", "office worker phone headset laptop", "business desk phone work laptop"],
        ...
        "fn": "woocommerce-b2b-portal-body3.jpg",
    },
]

def pexels_search(query, n=10):
    if not PEXELS_API_KEY:
        return []
    try:
        r = requests.get(
            "https://api.pexels.com/v1/search",
            params={"query": query, "orientation": "landscape", "per_page": n},
...
def openverse_search(query, n=10):
    for source in ["stocksnap", None]:
        try:
            params = {"q": query, "per_page": n, "aspect_ratio": "wide"}
            if source:
                params["source"] = source
            else:
                params["license"] = "cc0,pdm"
            r = requests.get("https://api.openverse.org/v1/images/", params=params, timeout=20)
            if r.status_code == 200:
...
def resize(data, w, h, q=82):
    ...
    if len(result) > 200*1024:
        ...
        result = buf.getvalue()
    print(f"  Resized {ow}x{oh} -> {w}x{h}: {len(result)//1024}KB")
    return result

def upload(data, fn, alt):
    hdrs = {"Authorization": WP_AUTH, "Content-Disposition": f'attachment; filename="{fn}"', "Content-Type": "image/jpeg"}
    print(f"  Uploading {fn} ({len(data)//1024}KB)...")
    for attempt in range(2):
        try:
            r = requests.post(WP_MEDIA, headers=hdrs, data=data, timeout=120)
            if r.status_code in (200, 201):
                m = r.json()
                mid = m.get("id")
                print(f"  Upload OK: ID={mid}")
                # Set alt
                requests.post(f"{WP_MEDIA}/{mid}",
...
def process_image(spec):
    ...
    if PEXELS_API_KEY:
        for q in spec["queries"][:2]:
            print(f"  Pexels: '{q}'")
            photos = pexels_search(q, 10)
            for p in photos:
                if p.get("width",0) >= spec["w"] and p.get("height",0) >= spec["h"]:
                    url = p["src"].get("large2x") or p["src"].get("original")
                    sid = str(p.get("id",""))
                    src = "pexels"
                    print(f"  -> Pexels ID {sid}")
                    break
            if url: break
            time.sleep(0.3)
    ...
    if not url:
        for q in spec["queries"]:
            ...
            for item in results:
                iu = item.get("url") or item.get("thumbnail")
                if iu and not iu.endswith(".gif"):
                    url = iu
                    sid = str(item.get("id","ov"))
                    src = "openverse"
                    print(f"  -> Openverse: {iu[:60]}")
                    break
            if url: break
            time.sleep(0.7)
    ...
    if not url:
        ...
        return {"name": spec["name"], "status": "FAILED", "media_id": None, "src_url": None, "alt": spec["alt"], "sid": None, "src": None}

    data = download(url)
    if not data:
        print(f"  Download FAILED")
        return {"name": spec["name"], "status": "DL_FAIL", "media_id": None, "src_url": None, "alt": spec["alt"], "sid": sid, "src": src}

    processed = resize(data, spec["w"], spec["h"])
    media = upload(processed, spec["fn"], spec["alt"])

    if media:
        return {"name": spec["name"], "status": "OK", "media_id": media.get("id"), "src_url": media.get("source_url",""), "alt": spec["alt"], "sid": sid, "src": src}
    return {"name": spec["name"], "status": "UPLOAD_FAIL", "media_id": None, "src_url": None, "alt": spec["alt"], "sid": sid, "src": src}


# ======== HTML BUILDER ========
SVG_ARROW = '<svg viewBox="0 0 24 24" width="18" height="18" style="fill:#43627f;" xmlns="http://www.w3.org/2000/svg"><path d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"/></svg>'
LI_STYLE = 'style="list-style:none!important;padding:8px 0 8px 32px!important;position:relative!important;line-height:1.5!important;margin:0!important;"'
SPAN_ARROW = f'<span aria-hidden="true" style="position:absolute!important;left:0!important;top:8px!important;">{SVG_ARROW}</span>'
A_STYLE = 'style="color:#00a0e2!important;text-decoration:none!important;font-family:metropolis,arial!important;font-size:16px!important;font-weight:500!important;"'

def toc_li(href, text):
    return f'<li {LI_STYLE}>{SPAN_ARROW}<a href="#{href}" {A_STYLE}>{text}</a></li>'
...
def build_html(imgs):
    b1 = img_span(imgs.get("body1", {}))
    b2 = img_span(imgs.get("body2", {}))
    b3 = img_span(imgs.get("body3", {}))
...
def push_post(html, featured_id):
    payload = {
    ...
    hdrs = {"Authorization": WP_AUTH, "Content-Type": "application/json"}
    print(f"\nPOSTing to {WP_POSTS}...")
    try:
        r = requests.post(WP_POSTS, headers=hdrs, json=payload, timeout=120)
        if r.status_code in (200, 201):
            p = r.json()
            print(f"OK: post ID={p.get('id')}, status={p.get('status')}")
            print(f"   link={p.get('link')}")
            return p
        print(f"FAIL {r.status_code}: {r.text[:400]}")
        time.sleep(5)
        r2 = requests.post(WP_POSTS, headers=hdrs, json=payload, timeout=120)
        if r2.status_code in (200, 201):
            p = r2.json()
            print(f"Retry OK: ID={p.get('id')}")
            return p
        print(f"Retry FAIL: {r2.status_code}: {r2.text[:200]}")
    except Exception as e:
        print(f"Push error: {e}")
    return None


def save_results(post, imgs, html):
    results_path = r"C:\content-intel\clients\virtina\output\research\publish_results.json"
    with open(results_path, "w") as f:
        json.dump({"post": post, "images": imgs, "html_len": len(html), "ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}, f, indent=2, default=str)
...
def main():
    ...
    print(f"User: {WP_USERNAME}, Pexels: {'YES' if PEXELS_API_KEY else 'NO'}\n")

    # Process images
    imgs = {}
    for spec in IMAGES:
        result = process_image(spec)
        imgs[result["name"]] = result
        time.sleep(0.5)

    print("\n=== IMAGE SUMMARY ===")
    for n, r in imgs.items():
        src = (r.get("src_url") or "")[:70]
        print(f"  {n}: {r['status']} | ID={r['media_id']} | pexels={r['sid']} | {src}")
    ...
    if "—" in html or "&mdash;" in html:
        print("WARNING: em dash found in HTML!")

    # Verify body image srcs
    for n in ["body1", "body2", "body3"]:
        src = imgs.get(n, {}).get("src_url") or ""
        if src and not src.startswith("https://virtina.com/wp-content/uploads/"):
            print(f"WARNING: {n} src not from virtina.com: {src[:80]}")

    # Push to WP
    featured_id = imgs.get("featured", {}).get("media_id") or 0
    print(f"\nFeatured media ID: {featured_id}")
    if featured_id == 0:
        print("WARNING: Featured image media_id=0 - flagging for manual set")

    post = push_post(html, featured_id)
    save_results(post, imgs, html)
    ...
    # Print summary
    failures = [n for n, r in imgs.items() if r["status"] != "OK"]
    if failures:
